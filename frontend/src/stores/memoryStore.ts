...
import { Memory, MemoryFilters, MemoryStatus } from '../types';
...
interface MemoryState {
  ...
  createMemory: (payload: Omit<Memory, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'status'>) => Promise<Memory | null>;
  updateMemory: (memoryId: string, payload: Partial<Memory>) => Promise<Memory | null>;
  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        // Actions
        fetchMemories: async (filters?: MemoryFilters) => {
          try {
        ...
        updateMemory: async (memoryId: string, payload: Partial<Memory>) => {
          try {
            set({ loading: true, error: null });
