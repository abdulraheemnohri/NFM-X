...
export class NFMApi {
  ...
  async buildContext(
    ...
    const response = await this.client.get<Context>('/api/v1/context', { params });
    return response.data;
  }
  ...
  async detectConflicts(): Promise<{ count: number }> {
    const response = await this.client.post<{ count: number }>('/api/v1/conflicts/detect');
    return response.data;
  }
  ...
  async deleteRelationship(sourceId: string, targetId: string): Promise<boolean> {
    await this.client.delete(`/api/v1/graph/relationships/${sourceId}/${targetId}`);
    return true;
  }
